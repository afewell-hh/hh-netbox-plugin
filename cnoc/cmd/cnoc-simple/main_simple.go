...
func main() {
	startTime = time.Now()
	log.Println("🚀 Starting CNOC - Cloud NetOps Command System (Enhanced)")
    ...
    <div class="container">
        ...
        <div class="cards">
            <div class="card">
                ...
                <p>CNOC system is operational with enhanced fabric management</p>
                <a href="/api/status" class="button">📊 API Status</a>
            </div>
            
            <div class="card">
                <h2><span class="status green"></span> Fabric Management</h2>
                <p>Manage Kubernetes fabrics with HNP equivalent features</p>
            ...
            <div class="card">
                ...
                <p>Core components status and health monitoring</p>
...
<head>
    ...
    <style>
        ...
        .fabric-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(400px, 1fr)); gap: 20px; margin-top: 20px; }
        ...
        async function loadFabrics() {
            try {
                ...
                if (data.results && data.results.length > 0) {
                    let html = '<div class="fabric-grid">';
                    data.results.forEach(function(fabric) {
                        ...
                        if (fabric.description) {
                            html += '<div class="meta-row"><span>Description:</span><span>' + fabric.description + '</span></div>';
                        }
    ...
    <script>
        ...
        async function loadFabricDetails() {
            try {
                ...
                let html = '<div class="fabric-overview">' +
                    '<h2>' + fabric.name + '</h2>';
                
                if (fabric.description) {
                    html += '<p>' + fabric.description + '</p>';
                }
                
                html += '<p><strong>Status:</strong> ' + fabric.status + '</p>' +
                    '<p><strong>Connection:</strong> ' + fabric.connection_status + '</p>' +
                    '<p><strong>Sync Status:</strong> ' + fabric.sync_status + '</p>' +
                    '<p><strong>CRDs:</strong> ' + fabric.cached_crd_count + '</p>' +
                    '<p><strong>Created:</strong> ' + new Date(fabric.created).toLocaleString() + '</p>' +
                    '<div>' +
                ...
                document.getElementById('fabric-content').innerHTML = html;
...
<head>
    ...
    <style>
        ...
        .button:hover { background: rgba(255,255,255,0.3); }
        .button.primary { background: rgba(34,197,94,0.7); border-color: rgba(34,197,94,0.9); }
        .crd-table { background: rgba(255,255,255,0.1); backdrop-filter: blur(10px); border-radius: 15px; padding: 20px; border: 1px solid rgba(255,255,255,0.2); overflow-x: auto; margin-top: 20px; }
        ...
        .status-badge { padding: 4px 8px; border-radius: 12px; font-size: 0.8em; font-weight: bold; background: rgba(34,197,94,0.7); }
        .loading { text-align: center; padding: 40px; font-size: 1.2em; }
    </style>
</head>
<body>
        ...
        async function loadCRDs() {
            try {
                ...
                if (data.results && data.results.length > 0) {
                    ...
                    data.results.forEach(function(crd) {
                    ...
                    content.innerHTML = 
                        '<div style="text-align: center; padding: 40px;">' +
                            '<h3>No CRDs found</h3>' +
                            '<p>No CRD resources available</p>' +
                        '</div>';
                }
            } catch (error) {
...
// API handlers (same as before, omitted for brevity but included in the actual file)
