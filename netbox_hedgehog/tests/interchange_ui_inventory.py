...
A status here is a security claim. ``source-located errors`` was marked
``known_false`` by #687 after a measured disclosure, then restored to
``asserted`` by #688 only with real-decoder evidence that the remediated
boundary holds. ``test_interchange_inventory_integrity`` keeps that claim tied
to observed decoder behaviour.
"""
...
UI_PASTE_INVENTORY = SeamInventory(
    ...
    notes=("#684 paste-only GREEN. Owner: Dev B; independent reviewer: Dev A; "
           ...
           "#687 recorded the measured source-location disclosure; #688 "
           "remediated it, and the paired behaviour check guards the restored "
           "assertion."),
    paths=(
