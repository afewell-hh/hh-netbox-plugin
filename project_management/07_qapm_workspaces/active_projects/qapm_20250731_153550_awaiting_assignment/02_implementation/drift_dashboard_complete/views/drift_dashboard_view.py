        ...
        try:
            ...
            for resource in drifted_resources:
                ...
                processed_resources.append({
                    ...
                    'can_sync': resource.fabric and resource.fabric.git_repository_url,
                })
                ...
                for fabric in HedgehogFabric.objects.all():
                    ...
                    fabric_drift_data.append({
                        ...
                        'avg_drift_score': fabric_resources.aggregate(
                            avg_score=models.Avg('drift_score')
                        )['avg_score'] or 0.0
                    })
...
class ResourceDriftDetailView(LoginRequiredMixin, TemplateView):
    """
    Detailed drift view for a specific resource
    """
    template_name = 'netbox_hedgehog/resource_drift_detail.html'
    ...
    def get_context_data(self, **kwargs):
        ...
        try:
            ...
            context['has_drift'] = drift_details.get('has_drift', False)
            
            # Process differences for display
            if drift_details.get('differences'):
                ...
                context['processed_differences'] = processed_diffs
            
            context['page_title'] = f'Resource Drift - {resource.kind}/{resource.name}'
            
        except Exception as e:
            logger.error(f"Resource drift detail error: {e}")
            context['error'] = str(e)
    ...
    def _format_diff_description(self, diff):
        ...
        descriptions = {
            ...
            'missing_desired': f"Missing in Git: {path}",
